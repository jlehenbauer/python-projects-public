...
def dailyIntake(caloricValue):
    options = []
    closest = 0
    for i in range(len(caloricValue)):
        closest = min([abs(2000 - sum(x)) for x in itertools.combinations(caloricValue, i)])
        options.append([x for x in itertools.combinations(caloricValue, i) if sum(x) == closest])
        print(closest)
    print(options)


dailyIntake([400, 800, 400, 500, 350, 350])